...
void HZZ4LeptonsAnalysis::Loop(Char_t *output)
{
   ...
   RoccoR  rc("/uscms/home/zwang4/nobackup/WORKSPCACE/ntuple/CMSSW_9_4_13/src/HiggsAnalysis/HiggsToZZ4Leptons/test/macros/roccor/RoccoR2017.txt"); //directory path as input for now; initialize only once, contains all variations
...
/*
 ...
 //             RECOMU_isMedium[i] &&//mediumID(qier)
          ...
          double mcSF = rc.kSmearMC(Q,Pt,Eta,Phi,nl,u1);          
...
//qier b-tag sf                       if( datasetName.Contains("DYJetsToLL")) sf_l=b_eff_dy_p5->GetBinContent(binxb);
//qier b-tag sf                       else if(datasetName.Contains("TT")) sf_l=b_eff_tt_p5->GetBinContent(binxb);
                       scaleFactor*=sf_l;
                      }
