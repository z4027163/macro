...
//  TGraph *mu_scale_factors_tk = (TGraph*)gDirectory->Get("ratio_eff_eta3_dr030e030_corr");
  ...
  TFile *b_scale_factors3_p5 = new TFile("btag/fake_mu_BH.root");
  TH1F *b_fk_p1 = (TH1F*)gDirectory->Get("hPtBot_9");
  TH1F *b_fk_p2 = (TH1F*)gDirectory->Get("f_ratio");
  TH1F *b_fk_p3 = (TH1F*)gDirectory->Get("hPtJet_9");
  TH1F *b_fk_p4 = (TH1F*)gDirectory->Get("f_ratio_j");
...
//      f_u1=-999, f_u2=-999;
      ...
      if( datasetName.Contains("DYJetsToLL")){
        zptweight=true;
        if(debug) cout << "DY correction" << endl;
      }
      newweight=weight;
	...
	pu_weight=double(puweight->GetBinContent(binx));
     ...
     if(!sm_trig) continue;
      ...
      double bmurate=-1;
      bmurate=0.0001;
      if( datasetName.Contains("DYBBJetsToLL")||datasetName.Contains("DYJetsToLL")){
        zptweight=true;
        if(dm_trig) bmurate=0.0001;//0.00024;
        if(de_trig) bmurate=0.00014;//0.00012;
      }
      if( datasetName.Contains("TT_TuneCUETP8M1")){
        if(dm_trig) bmurate=0.00006;//0.00024;
        if(de_trig) bmurate=0.000166;//0.00019;
      }

     if(bmurate<0) { cout << "INPUT ERROR"<<endl; break;}
...
/*
 ...
 //              RECOMU_isMedium[mu] &&//mediumID 
              ( RECOMU_isGlobalMu[mu] || RECOMU_isTrackerMu[mu] ) && RECOMU_isPFMu[mu] 
              && RECOMU_PT[mu] > 5. 
              && fabs(RECOMU_ETA[mu]) < 2.4 
              && fabs(RECOMU_mubesttrkDxy[mu]) < .5 && fabs(RECOMU_mubesttrkDz[mu]) < 1. //loose muon 
	      );
	  else continue;
...
//          if(RECOMU_CHARGE[ iL[j] ] != RECOMU_CHARGE[ iL[i] ]) continue;
	  ...
	  if(debug) cout << "2mu2e: " << Zxx_tag << endl; 
...
//          if(RECOELE_CHARGE[ iLe[j] ] != RECOELE_CHARGE[ iLe[i] ]) continue;
	...
	if(debug) cout << "Less than one Z pairs with isolated leptons...exiting" << endl;
	continue; 
      }
...
//        if (Zcandvector.at(index).massvalue <40) continue;
       if (Zcandvector.at(index).massvalue <10) continue;
...
//qier        if (leptonspTcleaned.at(0)>30. && leptonspTcleaned.at(1)>20.) {
...
//        }
     ...
     if (pTcleanedgoodZ.size()<1) {
        if(debug) cout << "No Z(PT) passing the mass cut"<< endl;
        continue;
      } 
      // Z1 selection
...
//no tk sf needed          if(mu_scale_factors_tk->Eval(Eta)>0) eff_weight*=tk_sf;
...
//            double sf_hlt = mu_scale_factors_hlt_p1->GetBinContent(binx3,biny3)*19.666/35.812+mu_scale_factors_hlt_p2->GetBinContent(binx32,biny32)*16.146/35.812;
            ...
            if (sf_hlt>0.) eff_weight*=sf_hlt;
            if(debug) cout << "l1trigger matching ele leg1 weight = " << sf_hlt << endl;
           }
         else{
            ...
            if (sf_hlt>0.) eff_weight*=sf_hlt;
            if(debug) cout << "l1trigger matching ele leg2 weight = " << sf_hlt << endl;
          }   
...
//qier        if(nomatch||noleg1) continue;
      }
      ...
      if( datasetName.Contains("DYBBJetsToLL")){
        if(Z1tag==1) bmurate=0.0001;//0.00024;
        if(Z1tag==2) bmurate=0.00014;//0.00012;
      }
      if( datasetName.Contains("TT_TuneCUETP8M1")){
        if(Z1tag==1) bmurate=0.00006;//0.00024;
        if(Z1tag==2) bmurate=0.000166;//0.00019;
      }
      ...
      f_z1_pt = ptZ1;
      if(debug) cout << "u1= " << f_u1 << " u2=" << f_u2 << " met=" << RECO_CORMETMUONS << endl;
...
*/
       if(abs(MC_Z_PDGID[0][1])==11&&MC_Z_PT[0][0]>=0){
        if(debug) cout << "MC ele PT=" << MC_Z_PT[0][0] << endl;
        if(MC_Z_PT[0][0]<5) newweight=newweight*1.05;
        else if(MC_Z_PT[0][0]<8) newweight=newweight*1.10;
        else if(MC_Z_PT[0][0]<10) newweight=newweight*1.06;
        else if(MC_Z_PT[0][0]<15) newweight=newweight*1.03;
        else if(MC_Z_PT[0][0]<20) newweight=newweight*1.00;
        else if(MC_Z_PT[0][0]<30) newweight=newweight*0.92;
        else if(MC_Z_PT[0][0]<40) newweight=newweight*0.93;
        else if(MC_Z_PT[0][0]<50) newweight=newweight*0.94;
        else if(MC_Z_PT[0][0]<65) newweight=newweight*0.96;
        else if(MC_Z_PT[0][0]<85) newweight=newweight*1;
        else if(MC_Z_PT[0][0]<150) newweight=newweight*1.02;
        }
       if(abs(MC_Z_PDGID[0][1])==13&&MC_Z_PT[0][0]>=0){
        if(debug) cout << "MC mu PT=" << MC_Z_PT[0][0] << endl;       
        if(MC_Z_PT[0][0]<5) newweight=newweight*1.06;
        else if(MC_Z_PT[0][0]<8) newweight=newweight*1.07;
        else if(MC_Z_PT[0][0]<10) newweight=newweight*1.05;
        else if(MC_Z_PT[0][0]<15) newweight=newweight*1.02;
        else if(MC_Z_PT[0][0]<20) newweight=newweight*0.97;
        else if(MC_Z_PT[0][0]<30) newweight=newweight*0.92;
        else if(MC_Z_PT[0][0]<40) newweight=newweight*0.90;
        else if(MC_Z_PT[0][0]<50) newweight=newweight*0.92;
        else if(MC_Z_PT[0][0]<65) newweight=newweight*0.94;
        else if(MC_Z_PT[0][0]<85) newweight=newweight*0.97;
        else if(MC_Z_PT[0][0]<150) newweight=newweight*1;
        }
      }
	...
	*/
	...
	if(debug) cout << "isMuon= " << ismuon << " and isElectron= " << iselectron << endl;

	if (ismuon){
	  for( int p = 0; p < Nphotons; ++p ){
	    if( iLp_l[ p ] == ipt[i] && iLp_tagEM[ p ] == 0 )  {
	      if(debug) cout << "Muon with pT= " << RECOMU_PT[ipt[i]] << " has associated a photon with pT= " << RECOPFPHOT_PT[iLp[p]] <<  endl;
	      flagFSR_tag=0;
	      pfsr=p;
	      break;
	    }
	  ...
	  for( int p = 0; p < Nphotons; ++p ){
	    if( iLp_l[ p ] == ipt[i] && iLp_tagEM[ p ] == 1 )  {
	      if(debug) cout << "Electron with pT= " << RECOELE_PT[ipt[i]] << " has associated a photon with pT= " << RECOPFPHOT_PT[iLp[p]] <<  endl;
	      flagFSR_tag=1;
	      pfsr=p;
	      break;
	    }
...
//	output_txt  << leptformat << endl;
	...
	if (flagFSR==1){
     ...
     int nbtag_pass=0;
     bool lead_jet=false;
     TLorentzVector JET1,JET2,BOT1,BOT2,BOT3,BOT4;
     ...
     int jet1=-999,jet2=-999,bot1=-999,bot2=-999,bot3=-999,bot4=-999;      
     int jetfail[100];
	 ...
	 if (jetfail[i]==0){
	   ...
	   if (njets_pass==2){
	     ...
	     JET2.SetPtEtaPhiE(RECO_PFJET_PT[i],RECO_PFJET_ETA[i],RECO_PFJET_PHI[i],RECO_PFJET_ET[i]*TMath::CosH(RECO_PFJET_ETA[i]));
           ...
           if (nbtag_pass==3&&cSV_BTagJet_DISCR[i]> 0.5426){
             bot3=i;
             BOT3.SetPtEtaPhiE(RECO_PFJET_PT[i],RECO_PFJET_ETA[i],RECO_PFJET_PHI[i],RECO_PFJET_ET[i]*TMath::CosH(RECO_PFJET_ETA[i]));
           }
           if (nbtag_pass==4&&cSV_BTagJet_DISCR[i]> 0.5426){
             bot4=i;
             BOT4.SetPtEtaPhiE(RECO_PFJET_PT[i],RECO_PFJET_ETA[i],RECO_PFJET_PHI[i],RECO_PFJET_ET[i]*TMath::CosH(RECO_PFJET_ETA[i]));
           }
	 }
     ...
     njets_pass=njets_pass-2;
     nbtag_pass=nbtag_pass-2; 

     newweight=newweight*bmurate; //qier //tt: 0.00012  dy:0.00007(mm) 0.00004(ee)
                                         //tt: 0.00028/24  dy:0.00015/0.00008
     double tem1,tem2,tem3;
     double u3 = gRandom->Rndm();
     int bins= b_fk_p2->GetNbinsX();
     double pscale=1;
     for(int kk=1;kk<bins;kk++){
           if(u3>=b_fk_p2->GetBinContent(kk)&&u3<b_fk_p2->GetBinContent(kk+1)){         
             pscale=b_fk_p2->GetBinCenter(kk);
             break;
           }
         }

     double massZ2=(BOT1+BOT2).M()*pscale;
     double ptZ2=(BOT1+BOT2).Pt()*pscale;
     double Y_Z2=(BOT1+BOT2).Eta();
     double Phi_Z2=(BOT1+BOT2).Phi();


     Z2P4.SetPtEtaPhiM(ptZ2,Y_Z2,Phi_Z2,massZ2);

     TLorentzVector L1P4,L2P4,L3P4,L4P4;
     L1P4.SetPtEtaPhiM(RECOMU_PT[indexlep1Z1],RECOMU_ETA[indexlep1Z1],RECOMU_PHI[indexlep1Z1],0.105);
     L2P4.SetPtEtaPhiM(RECOMU_PT[indexlep2Z1],RECOMU_ETA[indexlep2Z1],RECOMU_PHI[indexlep2Z1],0.105);
     L3P4.SetPtEtaPhiM(BOT1.Pt()*pscale,BOT1.Eta(),BOT1.Phi(),0.105);
     L4P4.SetPtEtaPhiM(BOT2.Pt()*pscale,BOT2.Eta(),BOT2.Phi(),0.105);
//qier
  //  if(dm_trig){
     double temZ1=(L1P4+L4P4).M();
     double temZ2=(L1P4+L3P4).M();
     double Phi_Z1;
     if(pow((temZ1-Zmass),2)+pow((temZ2-Zmass),2)<pow((massZ1-Zmass),2)+pow((massZ2-Zmass),2)){
        massZ1=temZ1;
        massZ2=temZ2;
        ptZ1=(L1P4+L4P4).Pt();
        Y_Z1=(L1P4+L4P4).Eta();
        Phi_Z1=(L1P4+L4P4).Phi();
        ptZ2=(L2P4+L3P4).Pt();
        Y_Z2=(L2P4+L3P4).Eta();
        Phi_Z2=(L2P4+L3P4).Phi();
        Z1P4=L1P4+L4P4;
        Z2P4=L2P4+L3P4;
     }
 //   }
     if(massZ2<40 || massZ1<40) continue;
//     if(!(massZ2<120&&massZ2>60)||!(massZ1<120&&massZ1>60)) continue;
...
//separate
//    if(!((massZ1>60&&massZ1<120&&Z1tag==1)||(massZ2>60&&massZ2<120&&Z1tag==2))) continue;
     ...
     if(ptZ2>ptZ1){
      ...
      hMZ1_5->Fill(massZ1,newweight*nbjetweight);
      hPtZ1_5->Fill( ptZ1,newweight*nbjetweight);
      hYZ1_5->Fill( Y_Z1,newweight*nbjetweight);

      hMZ2_5->Fill(massZ2,newweight*nbjetweight);
      hPtZ2_5->Fill( ptZ2,newweight*nbjetweight);
      hYZ2_5->Fill( Y_Z2,newweight*nbjetweight);

     hPFMET_8->Fill(RECO_CORMETMUONS,newweight*nbjetweight);
     ...
     double w_eff=1;
     if(nbtag_pass==1) w_eff=2;
     if(nbtag_pass==2) w_eff=3;

     Mjj_6->Fill(Mjj,newweight*w_eff);
     Mjj_6_mc_1->Fill(Mjj,newweight*mc_weight_un[1]*w_eff);
     Mjj_6_mc_2->Fill(Mjj,newweight*mc_weight_un[2]*w_eff);
     Mjj_6_mc_3->Fill(Mjj,newweight*mc_weight_un[3]*w_eff);
     Mjj_6_mc_4->Fill(Mjj,newweight*mc_weight_un[4]*w_eff);
     Mjj_6_mc_5->Fill(Mjj,newweight*mc_weight_un[5]*w_eff);
     Mjj_6_mc_6->Fill(Mjj,newweight*mc_weight_un[6]*w_eff);
     Mjj_6_mc_7->Fill(Mjj,newweight*mc_weight_un[7]*w_eff);
     Mjj_6_mc_8->Fill(Mjj,newweight*mc_weight_un[8]*w_eff);

     Mjj_6_up->Fill((JET1_up+JET2_up).M(),newweight*w_eff);
     Mjj_6_dow->Fill((JET1_dow+JET2_dow).M(),newweight*w_eff);

     Mjj_6_pdf_up->Fill(Mjj,newweight*(1+PDF_weighting_un)*w_eff);
     Mjj_6_pdf_dow->Fill(Mjj,newweight*(1-PDF_weighting_un)*w_eff);
     Mjj_6_pu_up->Fill(Mjj,newweight*pu_up*w_eff);
     Mjj_6_pu_dow->Fill(Mjj,newweight*pu_dow*w_eff);

     Mjj_6_jer_up->Fill((JET1_jer_up+JET2_jer_up).M(),newweight*w_eff);
     Mjj_6_jer_dow->Fill((JET1_jer_dow+JET2_jer_dow).M(),newweight*w_eff);
    
     if(nbtag_pass==0){
     hMZ1_61->Fill(massZ1,newweight*w_eff);
     hPtZ1_61->Fill( ptZ1,newweight*w_eff );
     hYZ1_61->Fill( Y_Z1,newweight*w_eff );

     hMZ2_61->Fill(massZ2,newweight*w_eff);
     hPtZ2_61->Fill( ptZ2,newweight*w_eff );
     hYZ2_61->Fill( Y_Z2,newweight*w_eff );

     Mjj_7->Fill(Mjj,newweight*w_eff);
     }
     if(nbtag_pass==1){
      Mbj_6->Fill(Mjj,newweight*w_eff);

      hMZ1_62->Fill(massZ1,newweight*w_eff);
      hPtZ1_62->Fill( ptZ1,newweight*w_eff );
      hYZ1_62->Fill( Y_Z1,newweight*w_eff);

      hMZ2_62->Fill(massZ2,newweight*w_eff);
      hPtZ2_62->Fill( ptZ2,newweight*w_eff);
      hYZ2_62->Fill( Y_Z2,newweight*w_eff);
     }
...
//     hDPHI_MAX_JET_MET_8->Fill(max_dphi_jet_met, newweight); 
     ...
     double Mbb = (BOT3+BOT2).M();
     double PTbb = (BOT3+BOT2).Pt();
     double Ybb = (BOT3+BOT2).Eta();
...
/*
  ...
  //     if(scaleFactor2<=0.0001) scaleFactor2=1;
       ...
       double scaleFactor1_up, scaleFactor2_up;
       if(RECOBOT_MatchingMCTruth[bot1+1]==2) scaleFactor1_up = reader.eval_auto_bounds("up",BTagEntry::FLAV_C, Eta1, Pt1);
       else if(RECOBOT_MatchingMCTruth[bot1+1]==1) scaleFactor1_up = reader.eval_auto_bounds("up",BTagEntry::FLAV_B, Eta1, Pt1);
       else scaleFactor1_up = reader.eval_auto_bounds("up",BTagEntry::FLAV_UDSG, Eta1, Pt1);
...
*/
     ...
     ptbb_6_mc_8->Fill(PTbb,newweight*nbjetweight*mc_weight_un[8]);
      ...
      hMZ2_7->Fill(massZ2,newweight*nbjetweight);
      hPtZ2_7->Fill( ptZ2,newweight*nbjetweight);
      hYZ2_7->Fill( Y_Z2,newweight*nbjetweight);
     ...
     double M2l2b = (Z2P4+BOT3+BOT2).M();
     double M4l2b = (Z1P4+Z2P4+BOT3+BOT2).M();
     ...
     if(debug) cout << "test final eff_weight= " << eff_weight << endl;
...
*/
   ...
   nEvent_4l->SetBinContent(10,N_6);
   ...
   nEvent_4l_w->SetBinContent(10,N_6);
   ...
   mettree->Write(0,TObject::kOverwrite);
   theFile->Write();
   theFile->Close();
  // finaltree->Write();
} // end main
