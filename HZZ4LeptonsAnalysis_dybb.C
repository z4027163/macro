...
//  TGraph *mu_scale_factors_tk = (TGraph*)gDirectory->Get("ratio_eff_eta3_dr030e030_corr");
  ...
  TFile *b_scale_factors3_p1 = new TFile("btag/DYsample.root");
  ...
  TFile *b_scale_factors3_p5 = new TFile("btag/fake_mu_DYBB.root");
	...
	iL_loose_mu[i]=-999.;
 	...
 	if(/* ( RECOMU_isGlobalMu[i] || (RECOMU_isTrackerMu[i] && RECOMU_numberOfMatches[i]>0) )
	  ...
	  if( fabs( RECOMU_IP[i] ) > max_Ip_loose_mu ) max_Ip_loose_mu = fabs( RECOMU_IP[i] ) ;
       ...
       if(RECO_PFJET_PT[j]>30. && fabs(RECO_PFJET_ETA[j])<2.4){
...
//            double sf_hlt = mu_scale_factors_hlt_p1->GetBinContent(binx3,biny3)*19.666/35.812+mu_scale_factors_hlt_p2->GetBinContent(binx32,biny32)*16.146/35.812;
      ...
      for(int i = 0; i < N_good; ++i){
        if (fabs(RECOMU_PFX_dB_new[iL[i]])>=0.20) continue;
        if(RECOMU_PT[ iL[i] ]<10) continue;
        if((iL[i]==Zcandvector.at(0).ilept1||iL[i]==Zcandvector.at(0).ilept2)&&Z1tag==1) continue;
        for(int j=0;j<RECO_PFJET_N;j++){
          ...
          if(RECO_PFJET_PT[j]>10. && fabs(RECO_PFJET_ETA[j])<2.5&&cSV_BTagJet_DISCR[j]> 0.5426){
        ...
        if (jetfail2[j]==0){
           int binx = b_fk_p1->GetXaxis()->FindBin(Pt);
         ...
         //           b_eff=b_eff_p3->GetBinContent(binxb);
           double ptx=5.0/Pt;
           int bxmin=b_fk_p2->GetXaxis()->FindBin(ptx);

           double scale_fkj=b_fk_p1->GetBinContent(binx)/b_eff/(1-b_fk_p2->GetBinContent(bxmin));
           z2weight=scale_fkj;
           jetid=j;
         ...
         if(Pt<10) continue;
          ...
          float pTphot=-999.;
          Lepton1.SetPtEtaPhiM(RECOMU_PT[iL[i]], RECOMU_ETA[iL[i]], RECOMU_PHI[iL[i]], 0.105);
          Lepton2.SetPtEtaPhiM(Pt, RECO_PFJET_ETA[j], RECO_PFJET_PHI[j], 0.105);
          DiLepton=Lepton1+Lepton2;
          massZ = DiLepton.M();
          massZ_noFSR = massZ;
          if (debug) cout << "Mass Z= " << massZ << endl;
          ...
          for( int p = 0; p < Nphotons; ++p ){
            if( iLp_l[ p ] == iL[i] && iLp_tagEM[ p ] == 1 )  {  // exit a photon associated to a lepton electron
              // evaluate the mass
              LeptonCorrection.SetPtEtaPhiM(RECOPFPHOT_PT[iLp[p]],RECOPFPHOT_ETA[iLp[p]],RECOPFPHOT_PHI[iLp[p]],0);
              Lepton1=Lepton1+LeptonCorrection;
              DiLepton=Lepton1+Lepton2;
          ...
          candidateZ *Z = new candidateZ;
          Z->massvalue=massZ;
          if(massZ<60||massZ>120) continue;//qier
          Z->ilept1=iL[i];
          Z->ilept2=j;
          Z->pt1=RECOMU_PT[iL[i]];
          Z->pt2=Pt;
...
// Plot non-jet distribution
     ...
     double Ybb = (BOT1+BOT2).Eta();
//     double btag_weight=1;
     newweight=newweight*2.0/3.0;  //1 fk bot no 2/3 due to charge bias 
